...
(function () {
  ...
  b.isPointInRect = function (f, g) {
    if (
      !(f instanceof window.BMapGL.Point) ||
      !(g instanceof window.BMapGL.Bounds)
    ) {
      return false;
    }
  ...
  b.isPointInCircle = function (e, h) {
    if (
      !(e instanceof window.BMapGL.Point) ||
      !(h instanceof window.BMapGL.Circle)
    ) {
      return false;
    }
    var i = h.getCenter();
  ...
  b.isPointOnPolyline = function (f, h) {
    if (
      !(f instanceof window.BMapGL.Point) ||
      !(h instanceof window.BMapGL.Polyline)
    ) {
      return false;
    }
    var e = h.getBounds();
  ...
  b.isPointInPolygon = function (o, l) {
    if (
      !(o instanceof window.BMapGL.Point) ||
      !(l instanceof window.BMapGL.Polygon)
    ) {
      return false;
    }
    var k = l.getBounds();
  ...
  b.getDistance = function (j, h) {
    if (
      !(j instanceof window.BMapGL.Point) ||
      !(h instanceof window.BMapGL.Point)
    ) {
      return 0;
    }
  ...
  b.getPolylineDistance = function (f) {
    if (f instanceof window.BMapGL.Polyline || f instanceof Array) {
      var l;
      if (f instanceof window.BMapGL.Polyline) {
        l = f.getPath();
      } else {
        l = f;
      }
  ...
  b.getPolygonArea = function (t) {
    if (!(t instanceof window.BMapGL.Polygon) && !(t instanceof Array)) {
      return 0;
    }
    var R;
    if (t instanceof window.BMapGL.Polygon) {
      R = t.getPath();
    } else {
      R = t;
    }
